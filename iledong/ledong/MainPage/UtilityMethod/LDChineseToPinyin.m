...
@implementation LDChineseToPinyin

+ (NSString *) getSectionTitle:(NSString *)string
{
    NSString *str;
    if( !string || 0 == [string length] )
        ...
        if(([string characterAtIndex:0] > 64 && [string characterAtIndex:0] < 91) ||
           ([string characterAtIndex:0] > 96 && [string characterAtIndex:0] < 123) ) {
            str = [[NSString alloc] initWithFormat:@"%c",[string characterAtIndex:0]];
        } else
            str = [[NSString alloc] initWithFormat:@"%c",pinyinFirstLetter((unsigned short)[string characterAtIndex:0])];
    }
