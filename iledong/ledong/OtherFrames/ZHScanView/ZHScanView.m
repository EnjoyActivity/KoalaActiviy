...
-(void)startcScanningQrcord{
    ...
    //    最上部view
    UIView* upView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth,ScanY + 50)];//80
    upView.alpha = 0.5;
    ...
    //底部view
    UIView *downView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(leftView.frame),  ScanY + ScanWidth + 50 , ScreenWidth-CGRectGetWidth(leftView.frame) * 2, ScreenHight -ScanY - ScanWidth - 50)];
    downView.alpha = 0.5;
...
-(void)scanAnimation
{
    if (_upOrdown == NO) {
        _num ++;
        _line.frame = CGRectMake((ScreenWidth - ScanWidth)/2, (50 + ScanY )+ 2*_num, ScanWidth, 3);
        ...
        _num --;
        _line.frame = CGRectMake((ScreenWidth - ScanWidth)/2, (ScanY+ScanWidth + 50)-2*_num, ScanWidth, 3);
        if (_num == 0) {
            _upOrdown = NO;
        }
    }
